...
const checkTests = async() => {
  const sendReminder = async(user: User | undefined, test: Test, event: TournamentEvent, tournament: Tournament) => {
    try {
      if (user && test) {
  ...
  for (const tournament of tournaments) {
    if (notArray(tournament.tournamentEvents)) continue;
    for (const event of tournament.tournamentEvents) {
      if (notArray(event.tests)) continue;
      for (const test of event.tests) {
        if (!test.finished) {
          ...
          // If time isn't up
          if (minuteDifference < event.minutes) continue;
